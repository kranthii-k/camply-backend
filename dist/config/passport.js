...
function generatePseudoUsername() {
...
passport_1.default.use(new passport_google_oauth20_1.Strategy({
    ...
    try {
        ...
        let user = await prisma_1.default.user.findUnique({ where: { googleId } });
        if (user) {
        ...
        let usernameExists = await prisma_1.default.user.findUnique({ where: { username } });
        let attempts = 1;
        while (usernameExists && attempts < 10) {
            username = generatePseudoUsername();
            usernameExists = await prisma_1.default.user.findUnique({ where: { username } });
            attempts++;
        }
        if (usernameExists) {
